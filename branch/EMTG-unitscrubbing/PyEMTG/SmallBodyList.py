...
class SmallBodyList(object):
    ...
    def find_bubble_targets(self, ReferenceState, ReferenceJD, RelativePositionFilterMagnitude, RelativeVelocityFilterMagnitude):
        #initialize the list of bodies
        list_of_acceptable_bodies = []
        
        #find the reference sun distance
        ReferenceSunDistance = math.sqrt(ReferenceState[0]**2 + ReferenceState[1]**2 + ReferenceState[2]**2) / 149597870.691
        ...
        for body in self.body_list:
           ...
           if body.Aphelion >= (ReferenceSunDistance + RelativePositionFilterMagnitude / 149597870.691) and body.Perihelion <= (ReferenceSunDistance - RelativePositionFilterMagnitude / 149597870.691):
               #second and third filters are on relative position and velocity, so we have to get the current state vector
               BodyR, BodyV = body.locate_body(ReferenceJD, 1.32712440018e+11, 149597870.691)
