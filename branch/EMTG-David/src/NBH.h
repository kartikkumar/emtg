...
#ifndef _EMTG_SOLVER_NBH
#define _EMTG_SOLVER_NBH
...
public:
	//constructor
	NBH();
	NBH(EMTG::problem* Problem_input);
...
private:
	void SortBin(vector<double>& X,double score,int cap);
	double NormOfDif(vector<double>& A,vector<double>& B);

	std::vector<vector<double>> Bin;
