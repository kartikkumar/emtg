...
namespace EMTG { namespace Solvers {
	...
	NBH::NBH(EMTG::problem* Problem_input)
	{
	...
	int NBH::initialize(EMTG::problem* Problem_input)
	{
	...
	void NBH::SortBin(vector<double>& X,double score,int cap)
	{
		proxKill = false;
		int offset = 0; // Used for both index of deletion and eraser flag
		if(cap>0 && score < Fit[cap-1])
			SortBin(X,score,cap-1);
		else
		{
	...
	double NBH::NormOfDif(vector<double>& A,vector<double>& B)
	{
		double tot = 0;
		for (int i = 0; i < nX; i++)
			tot += pow((B[i]-A[i]),2.0);
	...
	int NBH::seed(vector<double> seed_vector)
	{
		//reset the number of failures
		number_of_failures_since_last_improvement = 0;
	...
	{		
		...
		if(gen > genSw)
		{
			...
			double F_w = 0.85;
			double varType = DoubleDistribution(RNG);
			for (int i = 0; i < 3; i++)
				num[i] = -1;
			if(varType < hopType) // DE mutation
			{
				for (int j = 0; j < 3; j++)
				{
					int safety = 0;
					...
					} while((num[j]==num[(j+1)%3] || num[j]==num[(j+2)%3])); // Forces reselection if choice is duplicate -- now using duplicate bin knowledge
				}
				for (int j = 0; j < nX; j++)
				{
					double temp = Bin[num[0]][j] + F_w*(Bin[num[1]][j] - Bin[num[2]][j]);
					if(temp > 1.0)
						temp = 2.0-temp;
					if(temp < 0.0)
						temp = -1.0*temp;
			...
			{
				for (int j = 0; j < phasePts.size(); j++) // Construct trial by phase combination
				{
					int f = phasePts[j];
					int e = nX;
					if(j != phasePts.size()-1)
					{
						e = phasePts[j+1];
					}
					int c = capDist(RNG);
			...
			for (int i = 0; i < nX; i++)
			{
				...
				if(mutUse < mutPer)
				{
					...
					xTri[i] = xTri[i] + perturb;
				}
				Xtrial_scaled[i] = xTri[i]; // New
			}
		}
		else
		{
			for (int i = 0; i < nX; i++)
				Xtrial_scaled[i] = DoubleDistribution(RNG);
	...
	int NBH::print_archive_header(string filename)
	{
	...
	int NBH::print_archive_line(string filename, int linenumber)
	{
		//print the archive in CSV format
		//the final entry in each line is the fitness value
	...
	int NBH::write_sparsity(string filename)
	{
