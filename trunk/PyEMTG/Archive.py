...
class PathItem(object):
    def __init__(self, step_index, time_index, number_of_steps_since_last_restart, seconds_since_last_restart, fitness):
        self.clear()

        if step_index >= 0:
            self.setvalues(step_index, time_index, number_of_steps_since_last_restart, seconds_since_last_restart, fitness)
    ...
    def setvalues(self, step_index, time_index, number_of_steps_since_last_restart, seconds_since_last_restart, fitness):
    ...
    def write(self, outputfile):
        outputfile.write(str(int(self.step_index)) + ',' + str(self.time_index) + ',' + str(int(self.number_of_steps_since_last_restart)) + ',' + str(int(self.seconds_since_last_restart)) + ',' + str(self.fitness) + '\n')
...
class Path(object):
    def __init__(self, step_index, time_index, number_of_steps_since_last_restart, seconds_since_last_restart, fitness):
        ...
        if step_index >= 0:
            self.append(step_index, time_index, number_of_steps_since_last_restart, seconds_since_last_restart, fitness)

    def clear(self):
        self.PathItems = []

    def append(self, step_index, time_index, number_of_steps_since_last_restart, seconds_since_last_restart, fitness):
        self.PathItems.append(PathItem(step_index, time_index, number_of_steps_since_last_restart, seconds_since_last_restart, fitness))
...
class Archive(object):
    ...
    def assemble_paths(self):
        ...
        for linenumber in range(0,len(self.ArchiveItems[0].values)):
            ...
            if current_reset_count > reset_number:
                ...
                self.Paths.append(Path(current_step_count, current_timestamp, 0, 0, current_fitness))
            else:
                #if we are not on a new path, append this step's results to the current path
                self.Paths[-1].append(current_step_count, current_timestamp, current_step_count - reference_step_count, current_timestamp - reference_timestamp, current_fitness)
    ...
    def write_paths(self, output_file_name):
        ...
        for Path in self.Paths:
            ...
            outputfile.write('Step #, Timestamp (s), steps since path start, seconds sincee path start, fitness\n')
            current_path += 1
